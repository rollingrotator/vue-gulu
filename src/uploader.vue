<template>
  <div class="hots-uploader">
    ...
    <ol class="hots-uploader-fileList">
      <li v-for="file in fileList" :key="file.name">

        <template v-if="file.status === 'uploading'">
          <h-icon name="loading" class="hots-uploader-spin"></h-icon>
        </template>
        <template v-else-if="file.type.indexOf('image') === 0">
          <img class="hots-uploader-image" :src="file.url" width="32" height="32" alt="">
        </template>
        <temaplte v-else>
          <div class="hots-uploader-defaultImage"></div>
        </temaplte>


        <span class="hots-uploader-name" :class="{[file.status]: file.status}">{{file.name}}</span>
        <button class="hots-uploader-remove" @click="onRemoveFile(file)">x</button>
      </li>
    </ol>
